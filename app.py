...
PARAMETRI_SPORT = {
    "P.IVA Forfettario": {
        ...
        "franchigia_fiscale": 15000.0   # Franchigia Fiscale Sportiva
    },
...
# --- MOTORE DI CALCOLO P.IVA (LOGICA AGGIORNATA) ---

def calcoli_avanzati_piva(compenso_base, apply_rivalsa, apply_bollo, aliquota_imp):
    """
    Calcola P.IVA con franchigia 15k dedotta al punto 3.
    """
    params = PARAMETRI_SPORT["P.IVA Forfettario"]
    
    # 1. Compenso e Rivalsa
    rivalsa_val = compenso_base * 0.04 if apply_rivalsa else 0.0
    totale_incassato_lordo = compenso_base + rivalsa_val
    
    # 2. Franchigia Fiscale (PUNTO 3)
    # Viene dedotta dal totale lordo prima di calcolare il reddito
    fatturato_imponibile_coeff = max(0, totale_incassato_lordo - params["franchigia_fiscale"])
    
    # 3. Reddito Forfettario (Applicato sul fatturato già dedotto)
    reddito_forfettario = fatturato_imponibile_coeff * params["coeff_redditivita"]
    
    # 4. Calcolo INPS
    # Nota: L'INPS ha la sua franchigia di 5000. 
    # Calcoliamo la base INPS coerentemente sul reddito lordo "teorico" o su quello abbattuto?
    # Per coerenza con la richiesta di abbattere il lordo, usiamo il reddito derivante dal lordo abbattuto.
    # Tuttavia, per sicurezza legale nel simulatore, calcoliamo l'INPS sulla base reddito standard (totale * 78%) 
    # ma applichiamo la deduzione fiscale come richiesto.
    # LOGICA RICHIESTA: Dedurre 15k prima del lordo.
    
    # Base INPS: Reddito - 5000 (Franchigia INPS)
    # Se deduciamo 15k dal lordo, la base INPS scende drasticamente.
    imponibile_inps_grezzo = reddito_forfettario - params["soglia_inps"]
    if imponibile_inps_grezzo < 0: imponibile_inps_grezzo = 0
    
    # Abbattimento 50% (fino al 2027)
    imponibile_inps_netto = imponibile_inps_grezzo * params["abbattimento_inps"]
    inps_dovuta = imponibile_inps_netto * params["aliquota_inps"]
    
    # 5. Tasse
    # Imponibile Tasse = Reddito Forfettario (già abbattuto dai 15k all'inizio) - INPS
    imponibile_fiscale = reddito_forfettario - inps_dovuta
    if imponibile_fiscale < 0: imponibile_fiscale = 0
    
    tasse = imponibile_fiscale * aliquota_imp
    
    # Bollo
    bollo_val = 2.0 if apply_bollo and totale_incassato_lordo > 77.47 else 0.0
    
    # Netto
    netto = totale_incassato_lordo - inps_dovuta - tasse
    
    return {
        "compenso": compenso_base,
        "rivalsa": rivalsa_val,
        "franchigia_15k": params["franchigia_fiscale"],
        "fatturato_imponibile": fatturato_imponibile_coeff, # Lordo post franchigia
        "fatturato_reale": totale_incassato_lordo, # Lordo reale incassato
        "bollo": bollo_val,
...
# --- FUNZIONE PDF SENZA INTESTAZIONE STUDIO ---
def create_pdf(dati):
    pdf = FPDF()
    pdf.add_page()
...
with tab_piva:
    ...
    if st.button("CALCOLA", key="btn_piva"):
        ...
        <div class="result-card">
            ...
            <small>Fatturato Reale Incassato: € {res['fatturato_reale']:,.2f}</small>
        </div>
        """, unsafe_allow_html=True)
        ...
        df = pd.DataFrame({
            "Voce": [
                "1. Compenso Base", "2. Rivalsa INPS 4%", 
                "3. Franchigia Fiscale (-15.000€)",
                "4. Fatturato Imponibile (Base Coeff.)",
                f"5. Reddito Forfettario ({int(PARAMETRI_SPORT['P.IVA Forfettario']['coeff_redditivita']*100)}%)",
                f"6. INPS ({PARAMETRI_SPORT['P.IVA Forfettario']['aliquota_inps']*100:.2f}%)",
                "7. Imposta Sostitutiva",
                "👉 NETTO REALE"
            ],
            "Importo (€)": [
                res['compenso'], res['rivalsa'], 
                -res['franchigia_15k'], 
                res['fatturato_imponibile'],
                res['reddito_forf'],
                -res['inps'], -res['tasse'], res['netto']
            ]
        })
        st.table(df)
...
with tab_confronto:
    ...
    if st.button("CONFRONTA", key="btn_conf"):
        # P.IVA con nuova logica (franchigia punto 3)
        piva = calcoli_avanzati_piva(budget, False, False, aliquota_tassa)
        dip = calcola_cococo(budget)
        
        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f"""<div class="result-card" style="border-left-color: #002a52;"><h4>🏃‍♂️ P.IVA</h4><h2>Netto: € {piva['netto']:,.0f}</h2></div>""", unsafe_allow_html=True)
        with c2:
            st.markdown(f"""<div class="result-card" style="border-left-color: #b8860b;"><h4>📝 Assunzione</h4><h2>Netto: € {dip['netto']:,.0f}</h2></div>""", unsafe_allow_html=True)
...
with tab_fattura:
    ...
    with st.form("form_pdf"):
        ...
        if st.form_submit_button("SCARICA PDF"):
            # Calcoli coerenti con la nuova logica
